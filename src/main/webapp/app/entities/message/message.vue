<template>
  <div>
    <h2 id="page-heading" data-cy="MessageHeading">
      <span v-text="$t('demoJhVue973App.message.home.title')" id="message-heading">Messages</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('demoJhVue973App.message.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'MessageCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('demoJhVue973App.message.home.createLabel')"> Create a new Message </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && messages && messages.length === 0">
      <span v-text="$t('demoJhVue973App.message.home.notFound')">No messages found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('demoJhVue973App.message.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('email')">
              <span v-text="$t('demoJhVue973App.message.email')">Email</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'email'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('phone')">
              <span v-text="$t('demoJhVue973App.message.phone')">Phone</span>
            ...
            <th scope="row" v-on:click="changeOrder('message')">
              <span v-text="$t('demoJhVue973App.message.message')">Message</span>
            ...
            <th scope="row" v-on:click="changeOrder('file')">
              <span v-text="$t('demoJhVue973App.message.file')">File</span>
            ...
            <th scope="row" v-on:click="changeOrder('city')">
              <span v-text="$t('demoJhVue973App.message.city')">City</span>
            ...
            <th scope="row" v-on:click="changeOrder('otherCountry')">
              <span v-text="$t('demoJhVue973App.message.otherCountry')">Other Country</span>
            ...
            <th scope="row" v-on:click="changeOrder('date')">
              <span v-text="$t('demoJhVue973App.message.date')">Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('subject.name')">
              <span v-text="$t('demoJhVue973App.message.subject')">Subject</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="demoJhVue973App.message.delete.question" data-cy="messageDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-message-heading" v-text="$t('demoJhVue973App.message.delete.question', { id: removeId })">
