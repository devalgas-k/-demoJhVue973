<template>
  <div>
    <h2 id="page-heading" data-cy="ExpertiseHeading">
      <span v-text="$t('demoJhVue973App.expertise.home.title')" id="expertise-heading">Expertise</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('demoJhVue973App.expertise.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ExpertiseCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('demoJhVue973App.expertise.home.createLabel')"> Create a new Expertise </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && expertise && expertise.length === 0">
      <span v-text="$t('demoJhVue973App.expertise.home.notFound')">No expertise found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="$t('demoJhVue973App.expertise.title')">Title</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="$t('demoJhVue973App.expertise.description')">Description</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'description'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('level')">
              <span v-text="$t('demoJhVue973App.expertise.level')">Level</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="demoJhVue973App.expertise.delete.question" data-cy="expertiseDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-expertise-heading" v-text="$t('demoJhVue973App.expertise.delete.question', { id: removeId })">
